using Acubec.Solutions.Core;
using Acubec.Solutions.Core.Domain;
using Opus.Victus.SecurityInterface.Database;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace Opus.Victus.SecurityInterface.Domain
{
    internal class BaseServiceProvider : IDomainServiceProvider, IDisposable
    {
        protected SecurityDataContext _dbContext;
        protected IMapper _mapper;
        public BaseServiceProvider()
        {
            _dbContext = (SecurityDataContext)ContextFactory.Current.ResolveDependency<IDataContext>();
            _mapper = ContextFactory.Current.ResolveDependency<IMapper>();
        }

        #region IDisposable Support
        private bool disposedValue = false; // To detect redundant calls

        protected virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                if (disposing)
                {
                    // TODO: dispose managed state (managed objects).
                }

                // TODO: free unmanaged resources (unmanaged objects) and override a finalizer below.
                // TODO: set large fields to null.

                disposedValue = true;
            }
        }

        // override a finalizer only if Dispose(bool disposing) above has code to free unmanaged resources.
        ~BaseServiceProvider()
        {
            // Do not change this code. Put cleanup code in Dispose(bool disposing) above.
            Dispose(false);
        }

        // This code added to correctly implement the disposable pattern.
        void IDisposable.Dispose()
        {
            // Do not change this code. Put cleanup code in Dispose(bool disposing) above.
            Dispose(true);
            // TODO: uncomment the following line if the finalizer is overridden above.
            // GC.SuppressFinalize(this);
        }
        #endregion
    }
}
